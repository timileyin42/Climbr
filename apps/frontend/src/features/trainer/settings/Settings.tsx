...
function ProfileTab() {
  ...
  const { data: logoData } = useQuery({
    queryKey: ['trainer-logo'],
    queryFn: trainerApi.getLogo,
  })

  const upload = useMutation({
    mutationFn: (file: File) => trainerApi.uploadLogo(file),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['trainer-logo'] }),
  })
  ...
  return (
    <div className="space-y-5 max-w-sm">
      {/* Organisation logo */}
      <div>
        <label className="block text-[13px] font-[600] text-[var(--color-text-primary)] mb-2">Organisation logo</label>
...
const notifRows = ['New Enrolments', 'Application Status Change', 'Training Expiry Reminders', 'Credit Balance Alerts']
  ...
  return (
    <button
      ...
      className={cn(
        ...
        on ? '' : 'bg-[var(--color-border)]'
      )}
      style={on ? { background: 'var(--color-brand-yellow)' } : undefined}
    >
