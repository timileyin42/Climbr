...
async def employer_info(db: Session = Depends(get_db)):
    ...
    for package in pricing_packages:
        pricing_info.append({
            ...
            "description": (
                "Unlimited job posts · Annual · Saves ₦120,000 vs monthly"
                if package.quantity >= 9999 else
                "Unlimited job posts · Monthly subscription"
                if package.quantity >= 999 else
                f"Post {package.quantity} jobs · ₦{int(package.price // package.quantity):,} per post"
            )
        })
